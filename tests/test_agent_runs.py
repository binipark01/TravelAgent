...
def test_natural_language_request_creates_agent_run(
    ...
    response = client.post("/agent/runs", json=base_trip_payload)
    ...
    assert data["partial_plan"]["optimized_itinerary"]["days"]
    assert data["partial_plan"]["evidence_refs"]
    assert any(event["type"] == "source_discovered" for event in data["events"])
    assert any(event["type"] == "evidence_collected" for event in data["events"])
    assert any(event["type"] == "run_completed" for event in data["events"])
