...
# 지역 불문 한국인 실후기 소스 — 항상 함께 검색한다(커뮤니티 실후기·실제 코스가 핵심 요청).
_KOREAN_COMMUNITY = (
    "- 한국인 실후기(전 지역 공통): 네이버 카페 여행 후기글(cafe.naver.com), 디시인사이드 "
    "여행 갤러리(gall.dcinside.com '해외여행 갤러리' 등), 트리플(Triple) 후기, 네이버 블로그, "
    "마이리얼트립을 꼭 함께 검색하라. 특히 '○○ 2박3일 코스/일정 후기'처럼 실제 다녀온 사람의 "
    "동선·일정 글을 우선 참고해, 광고성 가이드보다 진짜 다녀온 한국인의 코스를 반영하라."
)
