...
def _nearby_block(nearby_options: list[str] | None) -> str:
    ...
    return (
        ...
        "시내만 채워라.\n"
        "단 먼 근교(편도 1.5~2시간 이상)는 실제 그곳을 당일치기로 다녀오는 게 흔할 때만 당일로 "
        "넣어라(막차 안). 보통 1박 하는 먼 곳(산속 호수·오지·먼 섬)이나 당일치기 근거가 없는 곳을 "
        "거리만 보고 하루 왕복으로 우겨넣지 마라 — 그런 곳은 빼라.\n"
    )
...
def curate_community_course(
    ...
    *,
    ...
    prompt = (
        ...
        "첫날(공항 도착)·마지막날(공항 출국)을 뺀 모든 날은 그 도시 여행자들이 보통 묵는 숙소 "
        "권역(시내 중심)에서 출발하는 흐름으로 짜라 — 그날 첫 stop을 '숙소 부근(○○ 일대)' 또는 "
        "출발 역(시내 주요 역, 예: 난바역·오페라역)으로 두고(duration 30 안팎), 거기서 첫 장소로 "
        "이동(travel)을 표시한 뒤 일정을 시작하라. 특히 근교·먼 구역 가는 날은 '숙소권역 출발 역 → "
        "그곳'으로 시작해 숙소에서 가는 이동을 분명히 보여라. 그리고 그날 마지막 stop도 다시 "
        "'숙소 부근(○○ 일대)'으로 돌아오게 끝내라 — 마지막 장소(저녁 포함)에서 숙소로 가는 이동을 "
        "표시하고, 그날 stops를 숙소로 시작해 숙소로 닫는다.\n"
        "하루 관광은 4~5곳으로 맞춰라(숙소 출발·복귀 stop은 이 수에 안 셈; 도시 명소를 하루에 "
        "몰아넣지 마라). 각 stop의 duration_min은 현실적으로 — 대형 미술관·궁전만 150~180분, 보통 "
        "명소 60~120분, 광장·전망·산책은 30~45분. 마지막 관광이 대략 18~19시에 끝나 저녁·숙소 "
        "복귀가 자정 전에 넉넉히 끝나게 분배하라(하루를 23시 넘게 끌지 마라).\n"
        "★ 야경·전망(나이트뷰)이 아니면 그날 모든 일정(관광·식사·이동·숙소 복귀)은 22시 안에 "
        "끝나야 한다. 특히 섬·근교 당일치기는 막배/막차 때문에 더 일찍 끝내고 곳수를 확 줄여라"
        "(욕심내지 마라 — 늦은 곳은 과감히 빼라). 22시를 넘길 것 같으면 반드시 곳수를 줄여 맞춰라. "
        "야경 명소만 22시 이후 허용한다.\n"
        "★ 먼 근교(편도 1.5~2시간 이상)는 후기에서 **사람들이 실제로 당일치기로 다녀오는지, "
        "거기서 1박 하는지**를 그대로 따라라. 후기 다수가 자고 온다면(산속 호수·오지·먼 섬 등) "
        "그날을 현지 1박으로 — 마지막 stop을 거기서 자는 '○○ 숙소/캠프 부근'으로 끝내고 다음 "
...
def arrange_itinerary(
    ...
    *,
    ...
    prompt = (
        ...
        "★ 야경·전망(나이트뷰)이 아니면 그날 모든 일정(관광·식사·이동·숙소 복귀)은 22시 안에 "
        "끝나야 한다. 특히 섬·근교 당일치기는 막배/막차 때문에 더 일찍 끝내고 곳수를 확 줄여라"
        "(욕심내지 마라). 22시를 넘길 것 같으면 반드시 곳수를 줄여 맞춰라. 야경만 22시 이후 허용.\n"
        ...
        "첫날·마지막날을 뺀 모든 날은 본거지(숙소 부근)에서 출발하는 흐름으로 짜라 — 그날 첫 "
        "stop을 본거지 또는 본거지 근처 출발 역으로 두고(duration 30 안팎), 거기서 첫 장소로 "
        "이동(travel)을 표시해 '숙소/역에서 출발'하게 하라(특히 근교 가는 날은 숙소권역 역에서 "
        "출발). 그리고 그날 마지막 stop도 본거지(숙소)로 돌아오게 끝내라 — 마지막 장소→숙소 "
        "이동을 표시하고 그날 stops를 숙소로 시작해 숙소로 닫는다.\n"
