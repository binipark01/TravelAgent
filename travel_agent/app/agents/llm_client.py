...
class CodexTripBriefClient:
    ...
    def extract_trip_brief(
        ...
        prompt = (
            ...
            "목적지 규칙: selected_destination은 일정의 거점이 되는 '단일 도시'로 정하라 — "
            "국가명·지역명·여러 곳을 이어붙인 문자열('이탈리아','일본 온천 지역','A + B','A·B')은 "
            "절대 넣지 마라. 여러 도시를 도는 여행이면 거점 도시 하나(보통 처음 묵거나 가장 큰 "
            "도시)를 selected_destination으로 하고, 방문 도시들은 destinations 배열에 각각 "
            "도시명으로 나열하라(국가명은 배열에 넣지 마라). 근교·랜드마크(그랜드캐니언·하롱베이·"
            "나라 등)는 destinations가 아니라 must_include에 넣어라. 모든 지명은 한국어로 적어라"
            "(라스베이거스·그랜드캐니언). 목적지를 콕 집지 않고 지역·분위기만 말했으면(예: '유럽',"
            "'따뜻한 데') selected_destination은 null로 두고 그 의도를 destination_hint에 담아라.\n"
            "접근성·거동(휠체어·유아차·배리어프리·고령자 동반)은 accessibility_needs 배열에, "
            "식이 제한(비건·채식·할랄·알레르기)은 dietary_restrictions 배열에 담아라"
            "(must_include에만 넣지 마라). 예산이 '1인당 N'이면 budget_per_person에만, "
            "'총 N'이면 budget_total에만 넣어라(둘 다 같은 값으로 채우지 마라).\n"
            f"통화: {currency}\n"
