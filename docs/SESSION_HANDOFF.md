...
## 8. 알려진 한계 / 후속 후보
- **[해결]** 트리플 코스 공항 이동시간 과대추정 → 공항 끼는 구간 고정 75분(`_store_travel`).
- **[해결]** 국내(제주·부산) 비자 카드 생략(visa 에이전트 도메스틱 감지). FX는 KRW→KRW라 이미
  None. 도메스틱 항공은 유효해 유지.
- **[보류] 항공 스크래퍼 가끔 0건**(다낭 1건): 브라우저 스크래퍼(google/naver) 경로라 재시도가
  느리고 검증이 어려워 보류. transient 재시도 적용하려면 스크래퍼 결과 캐시·재호출 설계 필요.
- **[보류] 멀티시티 트리플 코스 조합**: 멀티시티는 이미 웹코스로 도시를 정확히 돌고 `_store_poi`
  좌표 주입으로 트리플 좌표를 받음(피렌체 두오모 등 확인). 세그먼트별 공항 북엔드 조합은
  복잡도·회귀위험이 속도 이득보다 커서 보류(단일도시만 스토어 코스).
- 트리플 스토어 커버리지: 63도시·1~6일만(비슈케크·7일+는 웹검색 폴백).
- 트리플 POI 평점(metadata.reviewsRating)을 일정 카드에 노출하는 건 미구현(ItineraryItem에
  rating 필드 없음 — 스키마·프론트 작업 필요).
